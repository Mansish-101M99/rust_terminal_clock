    String::from("L6*3X9F!7T0o4d2^@7i7S5J8U2+T9pL4J!3m8r0k7&5X6Q@2r9t4B8!6w5^3i2d7p0o6S7F+7L2H42v5i3U8X9^&1n0L7Kj2T5Y8m2r+I%v4S6@3t7i9B6k^")
    ];
    ...
    fn what_to_print(front_space: &mut Vec<String>, hour_tens: &mut Vec<String>, hour_ones: &mut Vec<String>, space1: &mut Vec<String>,
        bigspace1: &mut Vec<String>, dot1: &mut Vec<String>, bigspace2: &mut Vec<String>, minute_tens: &mut Vec<String>,
        minute_ones: &mut Vec<String>, space2: &mut Vec<String>, bigspace3: &mut Vec<String>, dot2: &mut Vec<String>,
        bigspace4: &mut Vec<String>, second_tens: &mut Vec<String>, second_ones: &mut Vec<String>, space3: &mut Vec<String>,
    ...
    loop {
        clear_screen();
        let mut front_space: Vec<String> = my_strings.iter().map(|s| s[0..6].to_owned()).collect();
        let mut hour_tens: Vec<String> = my_strings.iter().map(|s| s[6..18].to_owned()).collect();
        let mut space1: Vec<String> = my_strings.iter().map(|s| s[18..22].to_owned()).collect();
        let mut hour_ones: Vec<String> = my_strings.iter().map(|s| s[22..34].to_owned()).collect();
        let mut bigspace1: Vec<String> = my_strings.iter().map(|s| s[34..39].to_owned()).collect();
        let mut dot1: Vec<String> = my_strings.iter().map(|s| s[39..41].to_owned()).collect();
        let mut bigspace2: Vec<String> = my_strings.iter().map(|s| s[41..46].to_owned()).collect();
        let mut minute_tens: Vec<String> = my_strings.iter().map(|s| s[46..58].to_owned()).collect();
        let mut space2: Vec<String> = my_strings.iter().map(|s| s[58..62].to_owned()).collect();
        let mut minute_ones: Vec<String> = my_strings.iter().map(|s| s[62..74].to_owned()).collect();
        let mut bigspace3: Vec<String> = my_strings.iter().map(|s| s[74..79].to_owned()).collect();
        let mut dot2: Vec<String> = my_strings.iter().map(|s| s[79..81].to_owned()).collect();
        let mut bigspace4: Vec<String> = my_strings.iter().map(|s| s[81..86].to_owned()).collect();
        let mut second_tens: Vec<String> = my_strings.iter().map(|s| s[86..98].to_owned()).collect();
        let mut space3: Vec<String> = my_strings.iter().map(|s| s[98..102].to_owned()).collect();
        let mut second_ones: Vec<String> = my_strings.iter().map(|s| s[102..114].to_owned()).collect();
        let mut backspace: Vec<String> = my_strings.iter().map(|s| s[114..].to_owned()).collect();
